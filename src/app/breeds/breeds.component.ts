...
export class BreedsComponent implements AfterViewInit {
  ...
  id: string='';
  breed: Observable<{breedDetails: Breed, breed_image_url: BreedImage, breed_images: BreedImage[]}>
  ...
  constructor(
    ...
    this.breed=this.autocompleteService.getBreed(this.id).pipe(
      switchMap((breedDetails: Breed) => {
        ...
        return combineLatest([img, imgs]).pipe(map(([breed_image_url, breed_images])=>({breedDetails, breed_image_url, breed_images})))
      })
